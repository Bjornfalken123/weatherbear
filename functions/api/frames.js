...
async function fetchFreshFrames(request) {
  ...
  if (requested && requested.getTime() > now.getTime() + 2 * 60 * 1000) {
    return {
      source: "SMHI",
      available: false,
  ...
  if (requested && now.getTime() - requested.getTime() > MAX_HISTORY_HOURS * 60 * 60 * 1000) {
    return {
      source: "SMHI",
      available: false,
  ...
  let frame = null;
  if (requested) {
    frame = await findNearestSmhiFrame(requested);
    if (!frame) {
      return {
        source: "SMHI",
        available: false,
  ...
  const imageFrame = await fetchRadarImageFrame(frame, requested || now);
...
async function getCachedResponse(context) {
  ...
  const keyTime = requested ? roundToNearestFiveMinutes(requested).toISOString() : "latest";
