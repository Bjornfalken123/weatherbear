export async function onRequestGet(context) {
  try {
    ...
    */
    ...
    let waterLevelForecastDiagnostic = null;
    ...
    async function getWaterLevelForecast(station) {
      ...
      try {
        ...
        if (!latestForecastFile) {
          waterLevelForecastDiagnostic = { ok: false, reason: "no_forecast_file_in_index" };
          return [];
        }
        ...
        if (!forecastResponse.ok) {
          waterLevelForecastDiagnostic = {
            ok: false,
            reason: "forecast_file_fetch_failed",
            status: forecastResponse.status,
            file: latestForecastFile,
            bodyPreview: forecastText.slice(0, 200)
          };
          throw new Error(`SMHI sea level forecast ${forecastResponse.status}: ${forecastText.slice(0, 200)}`);
        }
        waterLevelForecastDiagnostic = { ok: true, reason: "forecast_file_loaded", file: latestForecastFile };
        ...
        if (stationRowSeries.length) {
          waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: true, parser: "station_row", points: stationRowSeries.length };
          return stationRowSeries;
        }

        const wideSeries = seriesFromWideForecastTable(table, station);
        if (wideSeries.length) {
          waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: true, parser: "wide", points: wideSeries.length };
          return wideSeries;
        }
        ...
        if (stationRows.length) {
          const series = dedupeAndSortSeries(seriesFromForecastRows(stationRows));
          waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: !!series.length, parser: "station_rows", points: series.length };
          return series;
        }
        ...
        if (!rowsWithPosition.length) {
          waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: false, reason: "no_matching_station_or_position_rows" };
          return [];
        }
        ...
        if (!nearest || nearest.distanceKm > 50) {
          waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: false, reason: "nearest_forecast_row_too_far", distanceKm: nearest ? nearest.distanceKm : null };
          return [];
        }
        ...
        const series = dedupeAndSortSeries(seriesFromForecastRows(nearestRows));
        waterLevelForecastDiagnostic = { ...(waterLevelForecastDiagnostic || {}), ok: !!series.length, parser: "nearest_position_rows", points: series.length, distanceKm: nearest.distanceKm };
        return series;
      } catch (error) {
        console.warn("water level forecast failed", station.id, error);
        waterLevelForecastDiagnostic = waterLevelForecastDiagnostic || { ok: false, reason: "exception", message: error.message };
        return [];
      }
    }
    ...
    */
    ...
    return jsonResponse({
      ...
      sources: {
        ...
        waterLevel: waterLevelStation
          ? {
              ...
              forecastDiagnostic: waterLevelForecastDiagnostic
            }
          : null,
      ...
      series: {
        ...
        waterLevel: {
          ...
          note: waterLevelHistory.length || waterLevelForecast.length
            ? (waterLevelForecast.length ? "Färskaste användbara vattenståndsstation inom valt område används. Prognos hämtas från SMHI när matchning finns." : "Färskaste användbara vattenståndsstation inom valt område används. Prognos saknas i tillgänglig datakälla.")
      ...
      forecastNote:
        "Våghöjd och vattentemperatur har prognos från Open-Meteo Marine. Vattenstånd visar prognos endast när en tillgänglig SMHI-prognosfil kan hämtas och matchas.",
      availability: {
        waterTemp: { historyHoursRequested: 72, forecastHoursRequested: 168 },
        waveHeight: { historyHoursRequested: 72, forecastHoursRequested: 168 },
        waterLevel: {
          history: waterLevelHistory.length ? "station_observation" : "missing",
          forecast: waterLevelForecast.length ? "station_forecast" : "missing",
          forecastDiagnostic: waterLevelForecastDiagnostic
        }
      }
    });
